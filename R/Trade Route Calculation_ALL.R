# Calculations to formulate Trade Chain Index 
# for use in seizures rate bias correction
#
# This file is a slightly modified versin of Trade Route Calculation.R
# Modification is that it looks at all countries that have made a seizure
# Not just those included in the ETIS analysis for the TI index
#
# Trade Route Calculation.R is a modified version of a file that was used
...
#________________________________________________________________________________
# Selects worked in same way as raw
...
for (i in 1:8){
  nm.yr[[i]] <- list.rw[[i]][ , -4] %>%
    group_by(ctry, year) %>%
    summarise(y = n())
  nm.yr.all[[i]] <- expand.grid(ctry = as.character(df.ctry.used$code), year = year.from:year.to) %>%
...
setwd(path.data)
file.name <- paste("Ivory Route Ctry_Year un disc_", incl.disc, "_", year.from, "_", year.to, "_ALL.csv", sep = "")
